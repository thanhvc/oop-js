...
(function($, _, undefined) {
  ...
  var eXoMentions = function(settings) {
    ...
    function initTextarea() {
      ...
      //Get the DOM element parent
      elmInputWrapper = elmInputBox.parent();
      elmWrapperBox = $(settings.templates.wrapper());//templates.wrapper : _.template('<div class="exo-mentions"></div>'),
      elmInputBox.wrapAll(elmWrapperBox);//Wrap all the text area into the div elmWrapperBox
      elmWrapperBox = elmInputWrapper.find('> div');//Obtains the div elmWrapperBox that now contains the text area

      elmInputBox.attr('data-mentions', 'true');//Sets the attribute data-mentions-input to true -> Defines if the text area is already configured
      ...
      elmInputBox.on('keyup', onInputBoxKeyUp);//Bind the keyup event to the text area
      //elmInputBox.on('input', onInputBoxInput);
      elmInputBox.on('click', onInputBoxClick);//Bind the click event to the text area
      elmInputBox.on('paste', onInputBoxPaste);//Bind the paste event to the text area
    ...
    function initClickMention() {
      ...
      if (sp.length > 0) {
        $.each(sp, function(index, item) {
          ...
          //bind event click on x character
    ...
    function setCaretPosition(inputField) {
      if (inputField != null && inputField.length > 0) {
        ...
        if (inputField.val().length != 0) {
          ...
          //get selection object of browser
          var selection = getSelection();
          if (selection) {
            //add 3 characters spaces into the div cursorText
            //it's marked the old cursor befor lost focus
            cursorText.attr('contenteditable', 'true').css({
              'display' : 'inline',
              'height' : '14px'
            }).html('&nbsp;&nbsp;&nbsp;');
            //select 3 spaces character HTML
    ...
    function onInputBoxPaste(e) {
      //keep  old value of input box
      var before = $.trim(elmInputBox.value());
      //wait 50ms the data pasted to input box
      elmInputBox.animate({
        'cursor' : 'wait'
      }, 50, function() {
        var after = $.trim(elmInputBox.value());
        var info = utils.getIndexChange(before, after);
        //calculate the changes before and after pasted event
        ...
        //keep new line tag and clean the HTML tag 
    ...
    /**
    * Lost focus when click outside the composer inbox
    */
    ...
    function onInputBoxInput(e) {
      ...
      if (triggerCharIndex === 0) {
        if (isBlockMenu === false) {
          ...
          //valueBeforMention keep text before keydown
          var indexChanged = utils.getCursorIndexOfText(valueBeforMention, after);
          //get index change
          if (indexChanged > 0) {
            //get changed value and compares
    ...
    function onInputBoxKeyDown(e) {
      ...
      switch (e.keyCode) {
        case KEY.UP:
        case KEY.DOWN:
          //handle when KEY DOWN in auto-complete
    ...
    function checkRemoveMoreOneText(e) {
      ...
      if (delta > 1 && (isBlockMenu === true) || (isBlockMenu === false && delta == 1 && isBackKey)) {
        ...
        if (delta > textSizeMention && !utils.isFirefox) {
          //valueBeforMention get when key down
...
// keep committed id: e7afc555114f27e20acc343b35c56e609958c534
  ...
  $.fn.exoMentions = function(method, settings) {
    ...
    return this.each(function() {
      //keep eXoMentions selector to jquery.data
      //in the page, there is a lot of eXoMentions object, so it needs to keep in Jquery
      var instance = $.data(this, 'exoMentions') || $.data(this, 'exoMentions', new eXoMentions(settings));
      if (_.isFunction(instance[method])) {
        return instance[method].apply(this, Array.prototype.slice.call(outerArguments, 1));
      } else if (typeof method === 'object' || !method) {
        //here to call the init method of exomention object
